...
namespace Telegram.Core.MTProto.Crypto
{
    ...
    public class Factorizator
    {
        public static long FindSmallMultiplierLopatin(long what)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                int q = (Utils.Random.Next(128) & 15) + 17;
                long x = Utils.Random.Next(1000000000) + 1, y = x;
                int lim = 1 << (i + 18);
                for (int j = 1; j < lim; j++)
                {
                    long a = x, b = x, c = q;
        ...
        public static FactorizedPair Factorize(BigInteger pq)
        {
            if (pq.BitLength < 64)
            {
                ...
                throw new InvalidOperationException("Unable to factorize given number.");
        }
    }
}
