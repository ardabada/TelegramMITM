...
using Telegram.Core.Exceptions;
using Telegram.Core.MTProto;
using Telegram.Core.MTProto.Crypto;
using Telegram.Core.Network.Exceptions;
using Telegram.Core.Network.Requests;
using TeleSharp.TL;

namespace Telegram.Core.Network
{
    public class MtProtoSender
    {
        ...
        public async Task Send(byte[] packet, TeleSharp.TL.TLMethod request, CancellationToken token = default(CancellationToken))
        {
            ...
            using (MemoryStream plaintextPacket = makeMemory(8 + 8 + 8 + 4 + 4 + packet.Length))
            {
                using (BinaryWriter plaintextWriter = new BinaryWriter(plaintextPacket))
                {
                    ...
                    msgKey = Utils.CalcMsgKey(plaintextPacket.GetBuffer());
                    ciphertext = AES.EncryptAES(Utils.CalcKey(session.AuthKey.Data, msgKey, true), plaintextPacket.GetBuffer());
                }
            }
        ...
        private Tuple<byte[], ulong, int> DecodeMessage(byte[] body)
        {
            ...
            using (var inputReader = new BinaryReader(inputStream))
            {
                ...
                AESKeyData keyData = Utils.CalcKey(session.AuthKey.Data, msgKey, false);
